...
var Context = function () {
    ...
    // Put a dummy script element in our internal DOM. This really shouldn't be necessary, but some
    // apps unconditionally insert analytics scripts into the DOM by searching for an existing script
    // and inserting a new script after it. Having this script element here makes that insert work,
    // although the analytics script won't actually run because we only execute scripts that are
    // explicitly provided via runFiles.
    window.document.body.appendChild(window.document.createElement('script'));
